...
export default function AsciiArtPage() {
    const { isLoading } = useToolAccess("ascii-art");
    const { trackImmediate } = useToolTracking("ascii-art", { trackViewOnMount: true });
    ...
    const reportSecurityIncident = async (event: string, details: string) => {
        try {
            await fetch("/api/tools/public/ascii-art", {
                method: "POST",
    ...
    const handleDownloadTxt = () => {
        ...
        try {
            ...
            const safeName = fileName.replace(/\.[^/.]+$/, "").replace(/[^a-zA-Z0-9_-]/g, "_") || "ascii-art";

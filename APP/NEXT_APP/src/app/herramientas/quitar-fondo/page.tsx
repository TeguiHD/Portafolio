                                ...
                                <div className="min-w-0">
                                    <div className="flex flex-wrap items-center justify-between gap-2 border-b border-white/5 px-3 py-2">
                                        <div className="flex gap-1" role="group" aria-label="Vista de la imagen">
                                            {([{ id: "compare", label: "Antes / después", icon: ArrowLeftRight }, { id: "edit", label: "Retocar", icon: Paintbrush }, { id: "mask", label: "Máscara", icon: Layers2 }] as const).map(({ id, label, icon: Icon }) => <button key={id} className="studio-segment inline-flex items-center gap-1.5 whitespace-nowrap px-2.5" aria-pressed={view === id} aria-label={label} title={label} onClick={() => changeView(id)}><Icon size={14} aria-hidden="true" /><span className="hidden sm:inline">{label}</span></button>)}
                                        </div>
